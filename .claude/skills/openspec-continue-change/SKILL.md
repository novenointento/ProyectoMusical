...
1. **Determine input and get context**
   ...
   a. **If input looks like a Jira ticket ID** (matches pattern like `SCRUM-123`, `PROJ-456`, etc.):
      ...
      - **Derive a kebab-case change name from the ticket title**:
        ...
        - Example: "Actualizar Position API" → `update-position-api`, "Add User Auth" → `add-user-auth`
   ...
   Ejecutar `openspec list --json` to get available changes sorted by most recently modified. Then use the **AskUserQuestion tool** to let the user select which change to work on.

   Present the top 3-4 most recently modified changes as options, showing:
   - Change name
   - Schema (from `schema` field if present, otherwise "spec-driven")
   - Estado (e.g., "0/5 tasks", "complete", "no tasks")
   ...
   **If artifacts are ready to create** (status shows artifacts with `status: "ready"`):
   ...
   - **Crear the artifact file**:
     - **CRITICAL for tasks artifact**: If creating `tasks.md`:
       ...
       - All mandatory steps that MUST be included (e.g., Paso 0: Crear Feature Branch)
     ...
     - **For tasks artifact**: Ensure all mandatory steps from `config.yaml` and the rule file are included:
       - Paso 0: Crear Feature Branch (MUST be first step for backend changes)
       - Revisar and Actualizar Existing Unit Tests (MANDATORY)
       - Ejecutar Unit Tests and Verify Database State (MANDATORY)
       - Manual Endpoint Pruebas with curl (MANDATORY - AGENT MUST EXECUTE)
       - E2E Pruebas with Playwright MCP (MANDATORY if applicable - AGENT MUST EXECUTE)
       - Actualizar Technical Documentation (MANDATORY)
...
- **specs/*.md**: Crear one spec per capability listed in the proposal.
...
**Guardrails**
- Crear ONE artifact per invocation
